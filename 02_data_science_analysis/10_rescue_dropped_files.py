...
if __name__ == "__main__":
    ...
    print("Found {0:,} dropped files to re-evaluate with the resilient parser.".format(len(dropped_files)))

    rescued = 0
    if len(dropped_files) > 0:
        print("Launching Resumable Resilient Rescue across {0} cores...".format(multiprocessing.cpu_count()))
        
        batch_size = 5000
        processed_count = 0
        
        with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
            futures = {executor.submit(process_file, fp): fp for fp in dropped_files}
            ...
            for future in as_completed(futures):
                ...
                if processed_count % batch_size == 0:
                    conn.commit()
                    print("Evaluated {0:,}/{1:,} dropped files. Rescued: {2:,}".format(processed_count, len(dropped_files), rescued))
                    
            conn.commit()
            
    print("\nRescue Complete! Total files rescued: {0:,}".format(rescued))
